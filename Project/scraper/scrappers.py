import dlt, json, os, time, requests, sys
import pandas as pd
...
class reddit_scrapper(object):
    def __init__(self, headless=True, no_sandbox=True, disable_dev_shm_usage=True, days_extracted=3):
        ...
        # Point to system-installed chromedriver (already in /usr/local/bin)
    ...
    def get_posts(self):
        ...
        for post in tqdm(post_elements, file=sys.stdout):
        ...
        data = list(filter(lambda post: post['post_timestamp']>=datetime.now()-timedelta(days=self.days_extracted), data))
        print("Number of posts extracted: ", len(data))
    ...
    def get_comments(self):
        ...
        #iterate through the posts
        posts = self.posts_extracted
        for post in tqdm(posts, file=sys.stdout):
...
class cryptopanic_api_scrapper(object):
    ...
    def fetch_cryptopanic_feed(self):
        ...
        print("Number of posts extracted: ", len(self.posts_extracted))
    def get_comments(self):
        ...
        for post_id, title, published_at, url in tqdm(self.posts_extracted, file=sys.stdout):
            ...
            try:
                ...
                while comment_idx < comment_len:
                    ...
                    if comment_object not in comment_objects:
                        comment_object['comment_id'] = comment_idval
                        comment_idval+=1
            ...
            self.comments_extracted = comment_objects
            driver.quit()

    def yield_comments(self):
        for comment in self.comments_extracted:
            yield comment
